...
void Recorder_Init(Recorder *recorder) {
  ...
  // recorder->buffer = malloc(128 * snd_pcm_format_width(recorder->format) / 8 * 2);

  // fprintf(stdout, "buffer allocated\n");

  // for (int i = 0; i < 10; ++i) {
  //   if ((err = snd_pcm_readi(recorder->handle, recorder->buffer, recorder->buffer_frames)) != recorder->buffer_frames) {
